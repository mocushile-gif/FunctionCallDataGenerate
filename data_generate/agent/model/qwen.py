...
class QWen25Function:
    def __init__(self, url='', args={}):
        if url:
            self.url = f"http://{url}/generate"
        else:
            self.url=f"http://{os.environ['Qwen25_72B_URL']}/generate"
        self.parameters = {
            'max_new_tokens': 4096,
            'do_sample': False,
            "temperature": 0,
...
if __name__ == "__main__":
    ...
    messages=[
                {"role": "user", "content": "合并/mnt/nvme0/qinxinyi/function_call_data/data_generate/working_dir路径下的所有excel文件，并保存到原路径下，文件名是total_data"}
            ]
  ...
  "parameters": {
    ...
    #         add_generation_prompt=True,
        # ))
    model = QWen25Function("101.230.144.223:22223",{'temperature': 1.0, 'do_sample': True})
