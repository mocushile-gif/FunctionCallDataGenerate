...
class AddGroupByColumns(Tool):
    ...
    def execute(
        ...
        # 添加 Group By 字段
        if ctx.sql_parameters.group_by is None:
            ctx.sql_parameters.group_by = []

        ctx.sql_parameters.group_by.extend(columns)
    ...
    def describe() -> Dict[str, Any]:
        return {
            "type": "function",
            "function": {
                "name": "add_group_by_columns",
                "description": "Add GROUP BY columns to the current SQL query.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "database_path": {
