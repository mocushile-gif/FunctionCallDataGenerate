...
def filter_numeric_data(file_path: str, column_name: str, thresholds, sheet_name: str or int = 0, 
                        conditions=None, save_as: str = None,
                        logical_operator: str = "or",
                        ):

    """
    根据多个指定条件筛选 Excel 或 CSV 数据，并返回符合条件的行。

    参数:
    - file_path (str): Excel 或 CSV 文件的路径。
    - column_name (str): 用于筛选的列名，仅支持数值类型或日期类型列。
    - thresholds (list[float]): 筛选的阈值列表。
    - conditions (list[str], 可选): 筛选条件列表，支持 "greater_than", "less_than", "equal_to", 
      "greater_than_or_equal_to", "less_than_or_equal_to", "not_equal_to"。
    - save_as (str, 可选): 结果保存文件路径（支持 .xlsx 或 .csv）。
    ...
    if not (pd.api.types.is_numeric_dtype(column_dtype) or pd.api.types.is_datetime64_any_dtype(column_dtype)):
        return {"error": f"Unsupported data type for column '{column_name}'. Supported types are numeric and datetime."}
    ...
    if conditions is None:
        conditions = ["greater_than"] * len(thresholds)  # 默认使用 `greater_than`
    ...
    for threshold, condition in zip(thresholds, conditions):
        if condition == "greater_than":
            ...
            mask = df[column_name] >= threshold
        elif condition == "less_than_or_equal_to":
            mask = df[column_name] <= threshold
        elif condition == "not_equal_to":
            mask = df[column_name] != threshold
        else:
            return {"error": f"Unsupported condition '{condition}'. Supported conditions are 'greater_than', 'less_than', 'equal_to', 'greater_than_or_equal_to', 'less_than_or_equal_to', 'not_equal_to'."}
...
if __name__ == "__main__":
    ...
    result = filter_numeric_data(file_path, column_name, thresholds, sheet_name,conditions,logical_operator=logical_operator)
