...
def calculate_mean_confidence_interval(mean: float, std_dev: float, sample_size: int, confidence: float = 0.95, distribution: str = 'normal') -> tuple:
    """
...
if __name__ == "__main__":
    ...
    ci_normal = calculate_mean_confidence_interval(mean, std_dev, sample_size, confidence, distribution='normal')
    print(f"Normal Distribution CI: {ci_normal}")
    
    # For a t-distribution
    ci_t = calculate_mean_confidence_interval(mean, std_dev, sample_size, confidence, distribution='t')
