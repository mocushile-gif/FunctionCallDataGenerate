...
def get_database_info(database_name, work_dir=None):
    """
    Retrieve information about the SQLite database, including table names,
    schema details, record counts, and sample values.

    Parameters:
    - database_name (str): The name of the database (can include .db extension).
    - work_dir (str, optional): Custom working directory. Defaults to DATABASE_PATH env var.

    Returns:
    - dict: A dictionary containing table info: schemas, record counts, and sample values.
    """
    path = work_dir if work_dir else os.environ['DATABASE_PATH']
    ...
    for table in tables:
        ...
        # Try to fetch one sample row
        cursor.execute(f"SELECT * FROM {table} ORDER BY RANDOM() LIMIT 1;")
        sample_row = cursor.fetchone()
        sample_row = sample_row if sample_row else []

        # Build schema with sample values
        schema = []
        for idx, col in enumerate(schema_info):
            schema.append({
                "column_name": col[1],
                "data_type": col[2],
                "sample_value": sample_row[idx] if idx < len(sample_row) else None
            })
...
def read_pdf(pdf_path: str, preview_rows: int):
    ...
    - tool_agent: Optional tool agent instance for additional context.
...
database_info={
"airlines":"""Database name: airlines\n**Time Range:** 2017/06/21 to 2017/09/15\n**Description:** An operational dataset for a Russian airline covering various aspects such as aircraft, airports, flights, boarding passes, bookings, seats, and ticketing.""",
"chinook":"""Database name: chinook\n**Time Range:** 2009/01/01 to 2013/12/31  \n**Description:** A music and media dataset covering various aspects such as artists, albums, tracks, customers, employees, invoices, and playlists.""",
"olist_ecommerce":"""Database name: olist_ecommerce\n**Time Range:** 2017/01/01 to 2018/08/31  \n**Description:** An e-commerce dataset that covers various aspects from customers, orders, products, payments to reviews.""",
"uw_courses":"""Database name: uw_courses\n**Time Range:** 2006 to 2017  \n**Description:** A dataset from the University of Wisconsin - Madison that covers various aspects of university courses, including courses, course offerings, instructors, classrooms, and grade distributions.""",
"sakila":"Database name: sakila\n**Time Range:** 2005/05/24 to 2006/02/14  \n**Description:** A movie rental store operational dataset covering various aspects such as movies, actors, categories, inventory, rental records, customers, employees, and payment records.",
"NIPS_2015_Papers": "Database name: NIPS_2015_Papers\n**Time Range: 2015    \n**Description:** A structured database of all papers from the 2015 NIPS conference. It includes metadata (titles, abstracts, presentation types), full paper text, author information, and author-paper relationships.",
"OurAirports": "Database name: OurAirports\n**Time Range: updated in November 2024  \n**Description:** A comprehensive database offering public domain data on global aviation infrastructure, including airports, navigation aids, runways, and related metadata. It supports aviation research, mapping, and application development.",
}

def get_file_info(file_path: str, preview_rows: int = 5, work_dir=None):
    """
    ...
    - tool_agent (obj, 可选): 用于获取 Linux 服务器工作目录的对象
    ...
    try:
        if file_extension in (".xlsx", ".xls"):
            ...
            return {"response":
                    {"file_path": file_path,
                    "workbook_summary":extract_workbook_summary3b(file_path)}
            }
            ...
            # for database in database_info.keys():
            #     if database in file_path:
            database_schema=get_database_info(file_path,work_dir)
            return {"response": {"database_path":file_path,
                    "database_info":database_info.get(file_name,''),
        ...
        return {
            "response": {
                ...
                "num_columns": len(df.columns),
                "column_names": df.columns.tolist(),
                "random_preview_rows": df.sample(n=preview_rows).to_dict(orient="records")  # 预览随机 `preview_rows` 行
            }
        }
    except:
        # 格式或读取失败时退而求其次
...
if __name__ == "__main__":
    test_files = [
        # ("excel_data/CPU_分段合并.xlsx", 3),
        # ("./Avvai_Shanmughi.png", 10),
        ("./pdf_data/order.pdf", 10),
        # ("./image_data/portrait_diverse_women_different_ages.jpg", 10),
        # ("/mnt/afs2/qinxinyi/function_call_data/data_generate/working_dir/file_system_new/url.json",10),
        # ("/mnt/afs2/qinxinyi/function_call_data/data_generate/working_dir/file_system_new/output/nextjs_repository_page.md",10),
        # ("./excel_data/达人数据.xlsx",10)
        # ("/mnt/afs2/qinxinyi/function_call_data/data_generate/working_dir/shangtang_file_system/excel_data/财务付款申请单.xlsx",10)
        # ("database/IMDB_Movies_2021.db",10),
        # ("database/OurAirports_DataSource.sqlite",10),
        # ("database/bank_sales_trading.sqlite",10)
        # ("database/sakila.db",10)
